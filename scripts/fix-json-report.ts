/**
 * Script to fix the JSON data structure in the report
 */
...
async function fixJsonReport() {
  try {
    // Find the specific report being loaded
    const report = await prisma.userReport.findUnique({
      where: {
        id: 'bc42e003-936f-482c-a5a0-e2dedbcaa0ab'
      }
    })
    ...
    console.log('Current jsonData structure:', Object.keys((report.jsonData as any) || {}))

    // The correct JSON structure (not wrapped in success/data)
    ...
    // Update the report with correct structure
    const updated = await prisma.userReport.update({
      where: { id: 'bc42e003-936f-482c-a5a0-e2dedbcaa0ab' },
      data: {
    ...
    console.log('New structure has', correctJsonData.sections.length, 'sections')
    console.log('\n🎯 Now refresh: http://localhost:3000/reports/json/21989')
